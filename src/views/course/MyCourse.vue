...
<!--           <th>周一</th>
        ...
        <tbody>
        <tr v-for="(item2, index2) in 4" :key="'row-' + index2">
          <td v-if="item2 == 1" :key="index2">
            <p>{{ "第" + numberToChinease(item2) + "节" }}</p>
            <p>{{ "8:20" + " : " + "9:50" }}</p>
          </td>
          <td v-if="item2 == 2" :key="index2">
            <p>{{ "第" + numberToChinease(item2) + "节" }}</p>
            <p>{{ "10:20" + " : " + "12:00" }}</p>
          </td>
          <td v-if="item2 == 3" :key="index2">
            <p>{{ "第" + numberToChinease(item2) + "节" }}</p>
            <p>{{ "2:20" + " : " + "4:00" }}</p>
          </td>
          <td v-if="item2 == 4" :key="index2">
            <p>{{ "第" + numberToChinease(item2) + "节" }}</p>
            <p>{{ "4:30" + ":" + "6:00" }}</p>
          </td>
          <template v-for="(item3, index3) in weeks">
            <td
                :key="'row-' + index3"
                :rowspan="
                  showData(index3, index2 + 1).course &&
                  showData(index3, index2).course ===
                    showData(index3, index2 + 1).course
                    ? 2
                    : ''
                "
...
//import {weekCourse} from "@/views/course/Timetable";
...
export default {
  ...
  created() {
   ...
   // this.colorList = colorList;
    this.getCourse();
    const token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJ3b25pdXpzIiwiaWF0IjoxNzAxMTczNDQzLCJleHAiOjE3MDExNzUyNDMsImVpZCI6IjEwMDEiLCJlbmFtZSI6IuWwj-adqCIsIm5hbWUiOiJsb29seSJ9.CI81blz2IioFhMJlHm1hVZt7BeEAxeojgnyX1cjgFJ4';  
  ...
  methods: {
    //获取后端班级课程
    getCourse(){
      this.$axios.get("http://localhost:8081/course/get/all")
        .then(res=>{
